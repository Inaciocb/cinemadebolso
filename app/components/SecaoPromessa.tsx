          ...
          <div className="flex-1 space-y-8 text-center md:text-left w-full">
            ...
            <div className="grid grid-cols-1 gap-4 pt-4">
              {aprendizados.map((item, idx) => (
                <div key={idx} className="flex items-center gap-4 p-5 rounded-2xl bg-white/[0.05] border border-white/5 justify-start">
                  <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center shrink-0">
                    <Check size={18} className="text-green-500" />
                  </div>
                  <span className="text-base md:text-lg font-medium text-green-300 text-left">
                    {item}
                  </span>
                </div>
              ))}
