      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row gap-16 items-center">
          <div className="flex-1 space-y-8 text-center md:text-left">
            ...
            <div className="grid grid-cols-1 gap-4 pt-4">
              {problemas.map((item, idx) => (
                <div key={idx} className="flex items-center gap-4 p-5 rounded-2xl bg-white/[0.02] border border-white/5 group">
                  <XCircle size={26} style={{ color: CORES.error }} className="group-hover:scale-110 transition-transform" />
                  <span className="text-base md:text-lg font-medium text-zinc-300 group-hover:text-red-300 transition-colors">{item}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="flex-1 relative bg-zinc-900/80 p-10 md:p-14 rounded-[2rem] border border-white/10 backdrop-blur-xl italic text-center">
            <p className="text-2xl md:text-3xl font-light leading-snug"><span className="font-bold underline decoration-green-500 underline-offset-8" style={{ color: CORES.accent }}>A boa notícia é que isso pode ser aprendido!</span> Para um vídeo ficar bonito, é mais importante saber
